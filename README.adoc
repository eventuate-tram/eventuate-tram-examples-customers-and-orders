...
For more information, see the link:./customer-service-canvas.adoc[microservice canvas for the `Order Service`].
...
For more information, see the link:./customer-service-canvas.adoc[microservice canvas for the `Order History Service`].
...
docker-compose -f docker-compose-mysql-binlog.yml build
docker-compose -f docker-compose-mysql-binlog.yml up -d
```

Note:

1. You can also run the Postgres version using `docker-compose-eventuate-postgres-wal.yml`
